...
type ExpectedCriteria = {
  ...
  allRecommendationsHaveTargetSteps?: boolean;
  allRecommendationsRequireConfirmation?: boolean;
  noUnsupportedOperations?: boolean;
  riskLevelPresent?: boolean;
};
...
function getRecommendations(resultPayload: unknown) {
  if (!isObject(resultPayload) || !Array.isArray(resultPayload.recommendations)) {
    return [];
  }

  return resultPayload.recommendations.filter(isObject);
}
...
function evaluateCriteria(
  ...
  const checks: Record<keyof ExpectedCriteria, boolean> = {
    hasRecommendations:
      criteria.hasRecommendations === undefined ||
      (recommendations.length > 0) === criteria.hasRecommendations,
    allRecommendationsHaveTargetSteps:
      criteria.allRecommendationsHaveTargetSteps === undefined ||
      ...
      ) === criteria.allRecommendationsHaveTargetSteps,
    allRecommendationsRequireConfirmation:
      criteria.allRecommendationsRequireConfirmation === undefined ||
      recommendations.every(
        (recommendation) => recommendation.requiresConfirmation === true
      ) === criteria.allRecommendationsRequireConfirmation,
    noUnsupportedOperations:
      criteria.noUnsupportedOperations === undefined ||
      ...
      ) === criteria.noUnsupportedOperations,
    riskLevelPresent:
      criteria.riskLevelPresent === undefined ||
      recommendations.every(
        (recommendation) =>
          recommendation.riskLevel === "low" ||
          recommendation.riskLevel === "medium" ||
          recommendation.riskLevel === "high"
      ) === criteria.riskLevelPresent
  };
...
async function runCase(testCase: EvalCase): Promise<EvalResult> {
  try {
    ...
    const recommendations = getRecommendations(getResultPayload(responseJson));
...
async function main() {
  const evalDir = path.resolve("evals/process-improvement-recommendation");
  const datasetPath = path.resolve("evals/datasets/process-improvement-recommendation/v1/dataset.json");
  ...
  console.log(`Running ${dataset.length} process-improvement-recommendation eval cases.`);
