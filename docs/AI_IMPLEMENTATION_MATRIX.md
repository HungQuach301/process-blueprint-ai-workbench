...
| `ptr-to-brd-outline` | Module 3 - Product Delivery Core | `ProductDeliveryInput`: `ProcessTask[]`, optional project context, notes, source summary, generatedAt | Canonical `BRDResponse` inside validated `ProductDeliveryDraft` plus markdown preview/export | Yes. Deterministic generator creates structured BRD and markdown from the same draft. | Planned through V2 registry; not routed yet. | Planned through V2 registry; not routed yet. | Planned through V2 registry; not routed yet. | Strong for deterministic slice. `validateProductDeliveryDraft` calls canonical `validateBRD`, `validateSRS`, `validateUserStorySet`, and `validateAcceptanceCriteriaSet`. | Export Center Product Delivery section. | Preview first, then markdown download. No save/apply to Artifact Graph. Download is disabled until preview exists. | `export_product_delivery_draft` local audit on download with structured draft counts. Preview is not audited. | Needs route wiring, prompt execution, generation audit, and eventual Artifact Graph integration. |
| `ptr-to-srs-outline` | Module 3 - Product Delivery Core | `ProductDeliveryInput`: `ProcessTask[]`, optional project context, notes, source summary, generatedAt | Canonical `SRSResponse` inside validated `ProductDeliveryDraft` plus markdown preview/export | Yes. Deterministic generator creates structured SRS requirements from PTR task/gateway rows. | Planned through V2 registry; not routed yet. | Planned through V2 registry; not routed yet. | Planned through V2 registry; not routed yet. | Strong for deterministic slice through canonical `validateSRS`. | Export Center Product Delivery section. | Preview first, then markdown download. No save/apply to Artifact Graph. | `export_product_delivery_draft` local audit on download with SRS requirement count. | Needs route wiring, prompt execution, generation audit, and eventual Artifact Graph integration. |
| `brd-or-notes-to-user-stories` | Module 3 - Product Delivery Core | Planned BRD draft or notes, optional persona/module/scope | V2 target schema: `UserStorySetResponse`; current deterministic preview derives stories from PTR plus optional notes/context | No direct route skill yet. Deterministic Product Delivery preview includes canonical user stories. | Planned through V2 registry; not routed yet. | Planned through V2 registry; not routed yet. | Planned through V2 registry; not routed yet. | Canonical `validateUserStorySet` is active for deterministic Product Delivery draft; route skill remains planned. | Export Center text notes/context feed deterministic Product Delivery draft only. | Preview/export only in current deterministic Product Delivery slice. | Export audit only when Product Delivery markdown is downloaded. | Needs route, provider prompts, UI save/export model, and audit. |
| `ptr-to-user-stories` | Module 3 - Product Delivery Core | `ProductDeliveryInput`: `ProcessTask[]`, optional project context, notes, source summary, generatedAt | Canonical `UserStorySetResponse` inside validated `ProductDeliveryDraft` plus markdown preview/export | Yes. Deterministic as part of `generateProductDeliveryDraft`. | Planned through V2 registry; not routed yet. | Planned through V2 registry; not routed yet. | Planned through V2 registry; not routed yet. | Strong for deterministic slice through canonical `validateUserStorySet`. | Export Center Product Delivery preview. | Preview/export only. | `export_product_delivery_draft` local audit on download. | Needs route wiring for real AI generation and structured generation audit. |
| `user-stories-to-acceptance-criteria` | Module 3 - Product Delivery Core | Current deterministic path uses `ProductDeliveryInput` and PTR-derived user stories; planned route path uses user stories | Canonical `AcceptanceCriteriaResponse` inside validated `ProductDeliveryDraft` plus markdown preview/export | Yes. Deterministic as part of `generateProductDeliveryDraft`. | Planned through V2 registry; not routed yet. | Planned through V2 registry; not routed yet. | Planned through V2 registry; not routed yet. | Strong for deterministic slice through canonical `validateAcceptanceCriteriaSet`. | Export Center Product Delivery preview. | Preview/export only. | `export_product_delivery_draft` local audit on download. | Needs route wiring for real AI generation and structured generation audit. |
...
6. Product Delivery now has canonical structured deterministic drafts and markdown derived from the same validated model; real AI route wiring is still pending.
